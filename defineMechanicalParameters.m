...
syms link0h link0a link0V link0m link1h link1a link1b link1c link1V link1m link2h link2a link2b link2c link2V link2m link3a link3b link3c link3h link3V link3m f1 f2 f3 mu1 mu2 mu3  phi psii 'real' 
              ...
              % a         b       c       h     m
 parameters = [ link1a,  link1b,  0,      a1,  link1m        % R a  = ratio, h = length, m = mass
                link2a,  link2b,  0,      a2,  link2m        % R
                l3,      link3b,  link3c, 0,   link3m  ];    % P a = length, b = base, c = hight, m = mass
...
% inertial value of i-th motor over the z-axis
Im = zeros(1, dof, 'sym'); % = sym('Im', [1, dof], 'positive');

% gear rateos
kr = zeros(1, dof, 'sym'); % = sym('Kr', [1, this.dofs], 'positive');

% motor rotation axis w.r.t. its link 
% motor i is attached to link i-1
zm = sym([[0; 0; 1],[0; 0; 1],[0; 0; 1]]); 
